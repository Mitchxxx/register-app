	...
	environment {
     ...
     stages{
	    ...
	    stage("Trigger CD Pipeline") {
		    steps {
		        script {
		            sh '''curl -v -k --user admin:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data "IMAGE_TAG=${IMAGE_TAG}" 'ec2-34-248-8-148.eu-west-1.compute.amazonaws.com:8080/job/gitops-register-app-cd/buildWithParameters?token=gitops-token' '''
		        }
		    }
		}
     }
